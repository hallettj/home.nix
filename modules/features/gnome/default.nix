    ...
    let
      ...
      useOutOfStoreSymlinks =
        if builtins.hasAttr "useOutOfStoreSymlinks" config.home then
          ...
          false;
      dir = "${flakePath}/modules/features/gnome";
    in
    {
      imports = with self.modules.homeManager; [
        self.homeModules.gnomeExtensions
        paperwm
      ];
      ...
      xdg.configFile."mimeapps.list".source =
        if useOutOfStoreSymlinks then
          config.lib.file.mkOutOfStoreSymlink "${dir}/mimeapps.list"
        else
          ./mimeapps.list;
      ...
      dconf.settings = {
        ...
        "org/gnome/system/location" = {
          enabled = false;
        }; # Location required for automatic time zone
