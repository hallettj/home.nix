...
#
# For a usage example see modules/features/gnome/paperwm.nix
{
  flake.homeModules.paperwm =
    ...
    let
      ...
      buildWinprop =
        ...
        pipe winprop [
          ...
          (attrsets.mapAttrs' (
            name: value:
            if name == "wm-class" then
              nameValuePair "wm_class" value
            else if name == "preferred-width" then
              nameValuePair "preferredWidth" value
            else if name == "scratch-layer" then
              nameValuePair "scratch_layer" value
            else
    ...
    in
    {
      options.gnomeExtensions.paperwm = {
        ...
        cycle-width-steps = mkOption {
          type = with types; nullOr (listOf numbers.positive);
          default = null;
        };
        horizontal-margin = mkOption {
        ...
        vertical-margin-bottom = mkOption {
          type = with types; nullOr ints.unsigned;
          default = null;
        };
        window-gap = mkOption {
        ...
        show-window-position-bar = mkOption {
          type = with types; nullOr bool;
          default = null;
        };
        topbar-follow-focus = mkOption {
          type = with types; nullOr bool;
          default = null;
        };
      };
