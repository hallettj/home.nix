...
# extensions.
#
# For usage examples see modules/features/gnome/default.nix and
# modules/home-manager/paperwm.nix
{
  flake.homeModules.gnomeExtensions =
    {
      config,
      lib,
      pkgs,
    ...
    let
      ...
      enabledExtensionPackages =
        let
          ...
          uuidMatches =
            extensionPackage:
            let
              packageUuid = extensionUuid extensionPackage;
            in
          ...
          isEnabled = extensionPackage: builtins.any (uuidMatches extensionPackage) uuidsWithoutOverride;
        in
      ...
      extensionUuid =
        extensionPackage:
        let
          p = builtins.tryEval extensionPackage; # tryEval because some packages evaluate to deprecation errors
        in
        if p.success && builtins.isAttrs p.value && builtins.hasAttr "extensionUuid" p.value then
          p.value.extensionUuid
        else
          null;
      ...
      checkForMissing =
        ...
        in
        if missingUuids != [ ] then
          abort "Could not find packages for some Gnome extensions: ${builtins.toString missingUuids}"
        else
          extensionPackages;

      checkForExtensionUuids =
        extensionPackages:
        let
