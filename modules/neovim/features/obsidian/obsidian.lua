...
workspaces = vim.tbl_filter(
  function(workspace) return vim.fn.isdirectory(vim.fn.expand(workspace.path)) == 1 end,
  workspaces
)
...
require('lze').load {
  ...
  -- Load when opening a file in one of my Obsidian vaults
  event = vim.tbl_map(
    function(workspace) return 'BufReadPre ' .. vim.fn.expand(workspace.path) .. '/**/*.md' end,
    workspaces
  ),

  -- Or load when using one of the key bindings
  keys = {
  ...
  after = function()
    ...
    command('Link', function(data)
      ...
      if note_id ~= link_text then
        ...
        updated_line = string.sub(line, 1, cscol - 1) .. '[[' .. link_text .. ']]' .. string.sub(line, cecol + 1)
      end
    ...
    autocmd('DirChanged', {
      group = group,
      pattern = '*',
      callback = function(event) update_workspace(event.file) end,
    })
