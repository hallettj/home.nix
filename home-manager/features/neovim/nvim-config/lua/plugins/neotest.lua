return {
  ...
  keys = {
    { '<leader>tt', mode = 'n', function() require('neotest').run.run({ strategy = "dap" }) end, desc = 'run the nearest test' },
  ...
  config = function()
    require('neotest').setup {
      ...
      output_panel = {
        open = 'botright vsplit | vertical resize 80' -- open panel in vertical split
      },
    }
  end
}
